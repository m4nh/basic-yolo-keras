...
class YOLO(object):
    ...
    def NEEWcustom_loss(self, y_true, y_pred):
        mask_shape = tf.shape(y_true)[:4]
        ...
        # gt_class_pred = pred_box_class[...,true_box_class*(360.0/20.): true_box_class*(360.0/20.)+21 ]
        # loss_angle  =
        loss = tf.cond(tf.less(seen, self.warmup_batches+1),
                       lambda: loss_xy + loss_wh + loss_conf + loss_class + 10,
                       lambda: loss_xy + loss_wh + loss_conf + loss_class)
    ...
    def evaluate(self,
        ...
        for i in range(generator.size()):
            ...
            if len(pred_boxes) > 0:
                pred_boxes = np.array(
                    [[box.xmin, box.ymin, box.xmax, box.ymax, box.score] for box in pred_boxes])
            else:
                pred_boxes = np.array([[]])
